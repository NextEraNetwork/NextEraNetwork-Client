...
import { getUserEducation } from '@/services/operations/student/profileAPI';
...
const Education: React.FC<ProfileHeaderProps> = ({ username }) => {
                        ...
                        </div>
                        {
                            userEducation.map((item, index) => (
                                <div key={index} className="resume-item relative border-l-2 border-blue-500 pl-5 mb-6 before:content-[''] before:absolute before:w-4 before:h-4 before:rounded-full before:left-[-9px] before:top-0 before:bg-white dark:before:bg-slate-900 before:border-2 before:border-blue-500">
                                    <h4 className="text-lg font-bold uppercase text-gray-700 dark:text-gray-300">{item.degree} in {item.field_of_study}</h4>
                                    <h5 className="text-base font-semibold bg-accent/10 inline-block p-1 rounded-md mb-2 dark:text-gray-400">{moment(item.start_date).format('YYYY')} - {item.end_date ? moment(item.end_date).format('YYYY') : "Present"}</h5>
                                    <p className="italic text-gray-600 dark:text-gray-400 mb-4">{item.insitutionName}</p>
                                </div>
                            ))
                        }
                    </section>
